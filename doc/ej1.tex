\newpage
\section*{Task 1}
In this section NOT gates were implemented with BJT transistors (NPN). We used two different configurations,
 as shown below. 

\begin{figure}[H] 
\begin{center}
\includegraphics[width=0.70\textwidth]{data/1total.png}
\end{center}
...
First we meassured the \textit{logic levels}, where $V_{IH}$: minimum HIGH input voltage, $V_{IL}$: maximum LOW input voltage, $V_{OH}$: minimum HIGH output voltage, $V_{OL}$: maximum LOW output voltage.

To measure these values we use the ramp waveform and the oscilloscope in xy mode:
\begin{figure}[H] 
    \begin{center}
    \includegraphics[width=0.50\textwidth]{data/xy_ttl_bjt.png}
    \end{center}
    \caption{Characteristic logical levels}
    \label{fig:measures}
    \end{figure}     
As is shown in the picture, the values we are looking for are found in the ponts where the derivative is -1.
...
\subsection*{Case A1: RTL without load}
$V_{OL}$=244mV ;
$V_{OH}$=4,85V ; 
$V_{IL}$=547mV ;
$V_{IH}$=1,83V
